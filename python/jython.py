...
for i in range(0, estimate_count):
    ...
    coprime_probability = float(coprime_count) / float(pair_count)

    estimate = math.sqrt(6.0 / coprime_probability)
    estimate_sum += estimate
    coprime_count = 0

    print "Estimate %s: %s" % (i, estimate)

average_estimate = estimate_sum / estimate_count

print "Mean: %s" % average_estimate
